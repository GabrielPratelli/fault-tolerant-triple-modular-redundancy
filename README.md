...
The ESP32 runs channels A and B on its two cores, and the Arduino checks both of them.
...
The ESP32 is the main computational unit. Its two cores run channels A and B under FreeRTOS, each evaluating the same function with a different algorithm. It also holds its critical state in duplicate, so a corrupted copy can be detected and corrected.

The Arduino Uno is the supervisor. It evaluates the same function a third time with a lookup table, then arbitrates. Three results can agree unanimously, or the vote splits: majority on A, majority on B, a common-mode fault where two channels agree on a wrong answer, or all three different, and each case gets its own response. The supervisor sorts what it finds into a fault bucket, and once an error gets serious enough it calls a system lock that both boards hold until the fault clears.
...
1. **Open the Arduino project**: `File`, then `Open Folder`, then `~/Cose/TRM_System/arduino-supervisor`.
2. **Open the ESP32 project**: open a second VSCodium window (`Ctrl+Shift+N`) and use `File`, then `Open Folder`, then `~/Cose/TRM_System/esp32-supervisor`. Two separate windows means zero interference: each one only acts on its own folder and its own board.
3. On opening, PlatformIO initialises the project. A toolbar appears at the bottom with the build, upload and monitor buttons.
...
- The **Build** button at the bottom, or `pio run` from a terminal in that folder.
- Only the files in the current folder get compiled.

#### Upload to the right board

- Connect the board, then the **Upload** button.
- Each board has its own USB port:
  - Arduino Uno is usually `/dev/ttyACM0`, or `/dev/ttyUSB0` with a CH340 chip.
  - ESP32 is usually `/dev/ttyUSB0` (CP2102/CH340).
...
- The **Serial Monitor** button. You do not need to open the monitor for both boards at once in the same window, but always close the monitor before uploading on the same port, since the monitor keeps the port busy.
...
2. Build, fix any errors, then upload.
...
- The Arduino to ESP32 link uses the Uno's hardware UART (pin 0 = RX, 1 = TX) at 115200, not SoftwareSerial. Pins 0/1 are shared with the Uno's USB-serial chip, so disconnect the ESP32 (or the USB) while flashing the Arduino, otherwise the upload may get corrupted.
